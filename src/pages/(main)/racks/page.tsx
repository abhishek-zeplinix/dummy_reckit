...
const RackPage = () => {
    ...
    const onNewAdd = async (companyForm: any) => {
        const companyId = get(user, 'company.companyId');
        if (!validateNumber(companyForm.location)) {
            setAlert('error', "Please provide valid location");
            return;
        }
        if (!validateNumber(companyForm.rackTypeId)) {
            setAlert('error', "Please provide valid Rack Type");
            return;
        }
        if (!validateMaxNumberOfRacks(companyForm.noOfRacks)) {
            setAlert('error', "The racks can't be more than 50");
            return;
        }
        if (!validateMinNumberOfRacks(companyForm.noOfRacks)) {
            setAlert('error', "The racks must be atleast 1");
            return;
        }
        if (!validateNumberOfRackRow(companyForm.noOfRows)) {
            setAlert('error', "The Row must be atleast 1");
            return;
        }
    ...
    const onUpdate = async (companyForm: any) => {
        ...
        // //     return;
        // // }
        // if (!validateNumber(companyForm.rackType)) {
        //     setAlert('error', "Please provide valid Rack Type");
        //     return;
        // }
        // if (!validateMaxNumberOfRacks(companyForm.noOfRacks)) {
                                    ...
                                    >
                                        ...
                                        <div className="flex flex-column w-full surface-border p-3">
                                            <div className="flex align-items-center">
                                                ...
                                                <span>
                                                    This will permanently delete the selected Rack.
                                                    <br />
