...
import { buildQueryParams, getRowLimitWithScreenHeight, validateCountryCode, validateEmail, validateName, validateNumber, validatePhoneNumber } from '../../../utils/uitl';
...
if (!validateName(companyForm.name)) {
    setAlert('error', 'Please provide a valid Name');
    return;
}

// Validate company email
if (!validateEmail(companyForm.email)) {
    setAlert('error', 'Please provide a valid Email');
    return;
}

// Validate warehouse IDs
if (companyForm.warehouseIds.length < 1) {
    setAlert('error', 'Please provide a valid Warehouse');
    return;
}

// Validate category IDs
if (companyForm.categoryIds.length < 1) {
    setAlert('error', 'Please provide a valid Category');
    return;
}

// Validate all POCs (Points of Contact)
for (const poc of companyForm.pocs) {
    if (!validateName(poc.firstName)) {
        setAlert('error', 'Please provide a valid Contact Person Name');
        return;
    }
    if (!validatePhoneNumber(poc.phone)) {
        setAlert('error', 'Please provide a valid Contact Person Phone');
        return;
    }
    if (!validateEmail(poc.email)) {
        setAlert('error', 'Please provide a valid Contact Person Email');
        return;
    }
}

// Validate all addresses
for (const address of companyForm.addresses) {
    if (!validateName(address.type)) {
        setAlert('error', 'Please provide a valid Address Type');
        return;
    }
    if (!validateName(address.address1)) {
        setAlert('error', 'Please provide a valid Search Address');
        return;
    }
    if (!validateName(address.address2)) {
        setAlert('error', 'Please provide a valid Street Address');
        return;
    }
                                ...
                                <div className="p-fluid">
                                    <div style={{ width: '100%' }}>
                                        <div className="flex justify-content-between gap-3">
                                            ...
                                            <div className="field" style={{ width: '33.99%' }}>
                                            <label htmlFor="vendorName"> Vendor Name</label><span className="red">*</span>
                            ...
                            <Card title="Point Of Contact">
                                {form.pocs.map((poc, index) => (
                                    <div key={index} style={{ width: '100%', marginBottom: '1rem' }}>
                                        <div className="flex justify-content-between gap-3">
                                            <div className="field" style={{ width: '33.99%' }}>
                                                <label htmlFor={`pocFirstName-${index}`}>Contact Person Name</label><span className="red">*</span>
                                                <InputText
                                            ...
                                            <div className="field" style={{ width: '33.99%' }}>
                                                <label htmlFor={`pocPhone-${index}`}>Contact Person Phone</label><span className="red">*</span>
                                                <InputText
                                                    id={`pocPhone-${index}`}
                                            ...
                                            <div className="field" style={{ width: '33.99%' }}>
                                                <label htmlFor={`pocEmail-${index}`}>Contact Person Email</label><span className="red">*</span>
                                                <InputText
                                                    id={`pocEmail-${index}`}
